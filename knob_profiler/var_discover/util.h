...
void KPLog(const char *path, const char *fmt, ...) {
  ...
  if (!ofstr) {
    outs() << "Failed to open file: " << path << '\n';
    return;
  }
  // return;
  outs() << "Logging message: " << msg << " to file: " << path << '\n';
...
#define EXECUTIONPATH_FILE "/tmp/knob_profiler/executionPath.txt"

#ifdef SCHEMA_FILE
#define KPLogSchema(fmt, ...) KPLog(SCHEMA_FILE, fmt, ##__VA_ARGS__)
#else
#define KPLogSchema(fmt, ...)                                                  \
  do {                                                                         \
  } while (0)
#endif

#ifdef SRC2BASICBLOCK_FILE
#define KPSrc2Basicblock(fmt, ...)                                             \
  KPLog(SRC2BASICBLOCK_FILE, fmt, ##__VA_ARGS__)
#else
#define KPSrc2Basicblock(fmt, ...)                                             \
  do {                                                                         \
  } while (0)
#endif
#ifdef EXECUTIONPATH_FILE
#define KPExecutionPath(fmt, ...) KPLog(EXECUTIONPATH_FILE, fmt, ##__VA_ARGS__)
#else
#define KPExecutionPath(fmt, ...)                                              \
  do {                                                                         \
  } while (0)
#endif
...
record_precessors_successors(const BasicBlock *bb) {
...
void SaveBlockInfo(const BasicBlock *bb) {
  ...
  KPSrc2Basicblock("tag=%s,begin=%d,end=%d;", getSimplebbLabel(bb).c_str(),
                   beginLine, endLine);
  for (auto elem : seq)
...
void SaveFunctionInfo(const Function *f) {
  ...
  KPSrc2Basicblock("function=%s,begin=%d,end=%d,filename=%s\n",
                   demangledName.c_str(), visitedFunctions[f][0],
...
void SaveResult(Function *F,
  ...
  for (auto const &kv : result_vars) {
    ...
    KPLogSchema("%s %s %s %d %s %s %s\n", node->getDirectory().str().c_str(),
                node->getFilename().str().c_str(), demangledName.c_str(),
                node->getLine(), node->getName().str().c_str(), type.c_str(),
                kv.second.type.c_str());
  }
}
void SaveResult(
    Function *F, const std::unordered_map<Value *, MyMDNode> &result_vars,
  ...
  for (auto const &kv : result_vars) {
    ...
    if (!hasSuffix(filename, ".cc"))
      continue;
    KPLogSchema("%s %s %s %d %s %s %s", node->getDirectory().str().c_str(),
                node->getFilename().str().c_str(), demangledName.c_str(),
                node->getLine(), node->getName().str().c_str(), type.c_str(),
                kv.second.type.c_str());
    auto it = varToBB.find(kv.first);
    if (it != varToBB.end()) {
      for (const auto *BB : it->second) {
        KPLogSchema(" %s", getSimplebbLabel(BB).c_str());
      }
    }
...
void SaveGlobalResult(Module &m,
                      const std::unordered_set<GlobalValue *> &globals) {
  ...
  for (unsigned I = 0, E = CUNodes->getNumOperands(); I != E; ++I) {
    ...
    for (unsigned I = 0; I < GVs->getNumOperands(); I++) {
      auto *GV =
          dyn_cast_or_null<DIGlobalVariableExpression>(GVs->getOperand(I));
      if (!GV)
        continue;
      ...
      for (auto gVal : globals) {
        ...
        KPLogSchema("%s %s %s %d %s %s %s\n", Var->getDirectory().str().c_str(),
                    Var->getFilename().str().c_str(), "#global", Var->getLine(),
...
void SaveExecutionPath(
  ...
  for (auto const &kv : result_vars) {
    ...
    if (!hasSuffix(filename, ".cc"))
      continue;
    // 获取变量的执行路径
    auto it = varExecutionPaths.find(kv.first);
    if (it != varExecutionPaths.end()) {
      const std::vector<std::string> &executionPath = it->second;
      ...
      // 输出变量信息和执行路径
      KPExecutionPath("%s %s %s %d %s %s %s Execution Path: [%s]\n",
                      node->getDirectory().str().c_str(),
                      node->getFilename().str().c_str(), demangledName.c_str(),
                      node->getLine(), node->getName().str().c_str(),
                      type.c_str(), kv.second.type.c_str(), pathArray.c_str());
    } else {
      // 输出变量信息，没有执行路径
      KPExecutionPath("%s %s %s %d %s %s %s Execution Path: [None]\n",
                      node->getDirectory().str().c_str(),
                      node->getFilename().str().c_str(), demangledName.c_str(),
                      node->getLine(), node->getName().str().c_str(),
                      type.c_str(), kv.second.type.c_str());
    }
  }
}
...
std::string shell2CppRegex(const std::string &shellPattern) {
  ...
  while (pos != std::string::npos) {
    ...
    pos = cppPattern.find(
        '*', pos + 2); // Move to the. next occurrence, skipping the '*'
  }
  ...
  while (pos != std::string::npos) {
    cppPattern.replace(pos, 1, ".");
    pos = cppPattern.find(
        '?', pos + 1); // Move to the next occurrence, skipping the '?
  }
